...
\emph{Diffusing ideas} records a journey of research exploration and software development over several years which I have written for the interest of mathsy programmers and computational scientists. It's the output of many interrelated projects which seek to generalise the computational mathematics for the simulation, statistical inference, manipulation and automated control of stochastic phenomena as far as possible. As a consequence, the book accompanies a lot of new open-source scientific software written predominantly in \href{https://www.python.org/}{Python} and \href{https://go.dev/}{Go} --- all still very much in development.

My main motivation for creating these new computational tools is to prepare a core collection of modules for use as a foundation to develop new and more complex applications. I also hope that the resulting framework will enable anyone to explore and study new phenomena effectively, regardless of their scientific background.
...
%%
\begin{align}
S^{1}_{{\sf t}+1}(X',{\sf t}) &= \frac{(S^{0}_{{\sf t}+1} - S^{0}_{\sf t})}{\delta t({\sf t}+1)}\int^{t({\sf t}+1)}_{t({\sf t})}{\rm d}t' \frac{(t'-t)^{H-\frac{1}{2}}}{\Gamma (H+\frac{1}{2})} {}_2F_1 \bigg( H-\frac{1}{2};\frac{1}{2}-H;H+\frac{1}{2};1-\frac{t'}{t}\bigg) \label{eq:fbm} \,,
\end{align}
%%
we are able to reproduce the desired noise in the $i=1$ dimension of our state space, i.e., $S^{1}_{{\sf t}+1}(X',{\sf t})=[B_{H}]_{{\sf t}+1}$. The integral in Eq.~(\ref{eq:fbm}) can be approximated using an appropriate numerical procedure (like the trapezium rule). In the expression above, ${}_2F_1$ and $\Gamma$ are the ordinary hypergeometric and gamma functions, respectively.
