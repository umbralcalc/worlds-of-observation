...
\end{align}
%%
where we are assuming the state space is continuous in each dimension and $p_{{\sf t}+1}(X\vert X',{\sf t})$ is the conditional probability that next matrix will be $X$ given that the one in the previous timestep was $X'$. But what is $\Omega_{\sf t}$? You can think of this as just the domain of possible $X'$ inputs into the integral which will depend on the specific stochastic process we are looking at.

Given that it's only the most recent row of $X$ that gets updated in this timestep, it can also be useful to define the probability density matrix $P$ which has $x$-dependent elements $P^i_{\sf t}(x)$ --- this just means the probability density of $X^i_{\sf t}=x$. One then finds that the latest values of this matrix are updated by a step in time according to
%%
\begin{align}
...
\end{align}
%%
With another slight abuse of notation $V=X-X'$ and through Eq.~(\ref{eq:master-x-cont}), one also deduces that the probability density function for $V$ is
%%
\begin{align}
p_{{\sf t}+1}(V) &= \int_{\Omega_{{\sf t}}}{\rm d}X'p_{{\sf t}+1}(V+X' \vert X', {\sf t})p_{\sf t}(X') \,.
\end{align}
%%
