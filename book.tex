...
%%
\begin{align}
p_{{\sf t}+1}(X_{{\sf t}+1}) &= \int_{\Omega_{\sf t}}{\rm d}X' p_{\sf t}(X') p_{{\sf t}}(X_{{\sf t}+1}\vert X') \label{eq:master-x-cont} \,,
\end{align}
%%
where we are assuming the state space is continuous in each dimension and $p_{{\sf t}}(X_{{\sf t}+1}\vert X')$ is the conditional probability that next matrix row will be $X_{{\sf t}+1}$ given that the whole matrix in the previous timestep was $X'$. But what is $\Omega_{\sf t}$? You can think of this as just the domain of possible $X'$ inputs into the integral which will depend on the specific stochastic process we are looking at. For Markovian phenomena specifically, note that Eq.~(\ref{eq:master-x-cont}) simplifies back to a full master equation again (without conditions) which looks like this
%%
\begin{align}
p_{{\sf t}+1}(X_{{\sf t}+1}) &= \int_{\omega_{\sf t}}{\rm d}X_{{\sf t}} \,p_{\sf t}(X_{{\sf t}}) p_{{\sf t}}(X_{{\sf t}+1}\vert X_{{\sf t}}) \label{eq:master-x-cont-markov} \,,
\end{align}
%%
where $X_{{\sf t}+1}$ and $X_{\sf t}$ are referring to the next and previous rows of $X$, respectively. In Eq.~(\ref{eq:master-x-cont-markov}) we've also analogously defined $\omega_{{\sf t}}$ as the domain of $X_{{\sf t}}$ and, in this sense, $\omega_{{\sf t}}$ `exists within' $\Omega_{\sf t}$. The meaning of this is generally much clearer with a specific model in mind.

The matrix notation in Eq.~(\ref{eq:master-x-cont}) can hide some staggering complexity. To try and understand this a little, we can do a kind of Kramers-Moyal expansion~\cite{kramers1940brownian,moyal1949stochastic} of the conditional probability $p_{{\sf t}}(X_{{\sf t}+1}\vert X')$ which looks like this
%%
\begin{align}
p_{{\sf t}}(X_{{\sf t}+1}\vert X') &= p_{{\sf t}}(X_{{\sf t}}\vert X')-\sum_{i=1}^d(X^i_{{\sf t}}-X^i_{{\sf t}+1})\frac{\partial}{\partial X^i_{{\sf t}+1}}p_{{\sf t}}(X_{{\sf t}+1}\vert X') \nonumber \\
& \qquad + \frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d(X^i_{{\sf t}}-X^i_{{\sf t}+1})(X^j_{{\sf t}}-X^j_{{\sf t}+1})\frac{\partial}{\partial X^i_{{\sf t}+1}}\frac{\partial}{\partial X^j_{{\sf t}+1}}p_{{\sf t}}(X_{{\sf t}+1}\vert X') + \dots \label{eq:master-x-cont-kramers-moyal} \,,
\end{align}
%%
in which we have assumed that the state space is $d$-dimensional.
