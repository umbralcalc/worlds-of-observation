...
\alpha^{i}_1(X_{{\sf t}+1}) &= \sum_{{\sf t}'=0}^{\sf t}\int {\rm d}X^i_{{\sf t}'}(X^i_{{\sf t}'}-X^i_{{\sf t}+1})p_{\sf t}(X^i_{{\sf t}'}\vert X_{{\sf t}+1},{\sf t}+1) \\
\alpha^{ij}_2(X_{{\sf t}+1}) &= \sum_{{\sf t}'=0}^{\sf t}\sum_{{\sf t}''=0}^{\sf t}\int {\rm d}X^i_{{\sf t}'}\int {\rm d}X^j_{{\sf t}''}(X^i_{{\sf t}'}-X^i_{{\sf t}+1})(X^j_{{\sf t}''}-X^j_{{\sf t}+1})p_{\sf t}(X^i_{{\sf t}'}, X^j_{{\sf t}''}\vert X_{{\sf t}+1},{\sf t}+1) \,,
\end{align}
%%
...
It's easy to show that this more general expansion naturally simplifies into the more usually applied Kramers-Moyal expansion when considering a Markovian process --- you just remove the sums in the expressions for $\alpha_1$ and $\alpha_2$ and replace ${\sf t}'$ and ${\sf t}''$ both for ${\sf t}$ everywhere.
...
%%
\begin{align}
&p_{{\sf t}+1}(X_{{\sf t}+1}\vert X',{\sf t}) = \prod_{{\sf t}'=0}^{\sf t} (2\pi )^{-\frac{d}{2}}\vert K(\theta, X_{{\sf t}+1}, X_{{\sf t}'})\vert^{-\frac{1}{2}}{\rm exp}\Bigg[ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d\frac{f^i(X_{{\sf t}+1})f^j(X_{{\sf t}'})}{K^{ij}(\theta, X_{{\sf t}+1}, X_{{\sf t}'})}\Bigg] \label{eq:gaussian-proc-cond-p}\,,
\end{align}
%%
where $f(\cdot )$ is some arbitrary vector-valued function of the input state vector which contains expectation values of the Gaussian process and $K(\theta, X_{{\sf t}+1}, X_{{\sf t}'})$ is a covariance kernel matrix which also depends on the hyperparameters $\theta$. We write $\vert K(\theta, X_{{\sf t}+1}, X_{{\sf t}'}) \vert$ meaning its determinant. If we were to insert Eq.~(\ref{eq:gaussian-proc-cond-p}) into Eq.~(\ref{eq:master-x-cont-kramers-moyal}), after some further mathematical manipulations we would eventually find...


So the matrix notation of Eq.~(\ref{eq:master-x-cont}) can hide some staggering complexity! This example was just for illustration rather than using the Gaussian process for what it's intended for. However, it did highlight that the Kramers-Moyal-like expansion we wrote above is probably most usefully employed in situations with Markovian phenomena (where the expansion is more usually applied).
