...
My main motivation for developing these new computational tools is to prepare a core collection of modules to use as a foundation for developing new and more complex applications. I also hope that the resulting framework enables everyone to explore and study new phenomena both rigorously and efficiently, regardless of their scientific background.

The need to properly test all this new code has also provided a wonderful excuse to study, play with and invent new derivatives of an extensive range of mathematical toy models. I've chosen these models based on my broad background of interests, but also to illustrate the remarkable cross-disciplinary applicability of stochastic processes. Mathematical formalism can obscure what a programmer must tell their machine to do, so, while I have tried to be ambitious as possible with the level of technical sophistication in these models, I've also tried to write most of the expressions in a way so as to make their intended computations as explicit as possible.\footnote{E.g., think matrix instead of operator.} 
...
\end{align}
%%
When modelling continuous sample paths, it's also nearly always the case with mathematical models of real-world systems that the deterministic part will at least contain the term $D^{i}_{t+1}(X',t) = X^i_t$ because the overall system is a described as some stochastic differential equation. This is not a really requirement in our general formalism, however.
...
In another example, for \emph{Geometric Brownian motion noise} we would simply have
%%
\begin{align}
...
%%
\begin{align}
& F^{i}_{t+1}(X',t) = X^i_{t} + B^i_{t+1}({H_i})-B^i_{t}({H_i})\,.
\end{align}
%%
...
\section{\sffamily Analysis concepts}

Before progressing further, it will be helpful to discuss some useful concepts that should help us analyse the system later on. This section may be of most interest to the more mathematically-interested readers but I'm going to stick with the linear algebra style of formalism in this section so that the computations will hopefully be clear to other programmers as well.

Our general stochastic process, which we defined with Eq.~(\ref{eq:x-step-def}), also has a corresponding master equation~\cite{van1992stochastic} that describes the time evolution of its probability matrix elements
%%
\begin{align}
P^i_{t+1}(x) &= \int_{\Omega (X')} {\cal D}{\sf P}(X') P^i_{t+1}(x\vert X') \label{eq:master-x-cont} \,,
\end{align}
%%
where we are assuming the state space is continuous in each dimension. In the above relation $P^i_{t+1}(x)$ denotes the probability that $X^i_{t+1}=x$ and 
%%
\begin{align}
{\cal D}{\sf P}(Y) &= \prod_{\forall i}\prod^t_{s'=s} {\rm d}y \, P(Y^i_{s'}=y) \,,
\end{align}
%%
denotes the probability measure over the whole matrix of histories back to initial time step $s$. Using Eq.~(\ref{eq:x-step-def}) may also deduce that the conditional probability $P^i_{t+1}(x\vert X')=\delta [x-F^i_{t+1}(X',t)]$ where $\delta$ is a Dirac delta distribution.

Note that the analog of Eq.~(\ref{eq:master-x-cont}) for discrete state spaces simply replaces the integral with a sum like so 
%%
\begin{align}
P^i_{t+1}(x) &= \sum_{\Omega (X')} {\sf P}(X') P^i_{t+1}(x\vert X') \label{eq:master-x-disc} \,.
\end{align}
%%
