...
%%
\begin{align}
P_{{\sf t}'}(x) &= {\sf MultivariateNormalPDF}[x;f({\sf t}'),\Sigma (\theta , {\sf t}')]\label{eq:stat-sol-kramers-moyal}\,.
\end{align}
%%
...
It's possible to take this derivation a bit further by expanding Eq.~(\ref{eq:joint-master-x-cont}) in a similar fashion, truncating it to second-order, assuming only time-dependent terms and solving it in the stationary limit. If we then plugged this solution and Eq.~(\ref{eq:stat-sol-kramers-moyal}) into Eq.~(\ref{eq:cond-master-x-cont}) we would be able to derive a conditional probability that looks something like this
%%
\begin{align}
P_{({\sf t}+1){\sf t}'}(x\vert x') &= {\sf MultivariateNormalPDF}[x;f({\sf t}'),K(\theta , {\sf t}+1,{\sf t}')]\label{eq:stat-sol-kramers-moyal-cond}\,,
\end{align}
%%
where $K(\theta , {\sf t}+1,{\sf t}')$ is some arbitrary covariance matrix that encodes how the correlation structure varies with the length of time between compared states. Eq.~(\ref{eq:stat-sol-kramers-moyal-cond}) may look familiar to some readers who are familiar with Gaussian processes from the machine learning literature~\cite{murphy2012machine}.
