...
\end{align}
%%
we can reproduce the desired noise in the $i=1$ dimension of our state space. The integral in Eq.~(\ref{eq:fbm}) can be approximated using an appropriate numerical procedure (like the trapezium rule). Also notice that we needed to make more explict use of the continuous time variable $t$ and the iterations we are making with it at each timestep $\delta t({\sf t})$.
