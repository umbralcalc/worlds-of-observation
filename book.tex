...
\beta^{ij}_{({\sf t}+1){\sf t}'}(x) &= \Sigma^{ij}(\theta, {\sf t}') \,,
\end{align}
%%
where $f({\sf t}')$ is an arbitrary vector-valued function of the timestep and $\Sigma (\theta ,{\sf t}')$ is an arbitrary covariance matrix which depends on both the timestep and a set of hyperparameters $\theta$. If we now also assume stationarity of $P_{{\sf t}'}(x)=P_{{\sf t}''}(x)$ for any ${\sf t}'$ and ${\sf t}''$ such that
%%
\begin{align}
...
%%
\begin{align}
P_{{\sf t}'}(x) &= (2\pi )^{-\frac{d}{2}}\vert \Sigma(\theta, {\sf t}')\vert^{-\frac{1}{2}}{\rm exp}\bigg\{ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d[x-f({\sf t}')]^i[\Sigma^{-1}(\theta, {\sf t}')]^{ij}[x-f({\sf t}')]^j\bigg\}\label{eq:stat-sol-kramers-moyal}\,.
\end{align}
%%
Note that this solution also required the restriction of probability flow between timesteps such that $J_{({\sf t}+1){\sf t}'}(x)$ uniquely vanishes for each and every ${\sf t}'$. I also write $\vert \Sigma (\theta, {\sf t}') \vert$ meaning the determinant of the matrix and $\Sigma^{-1}(\theta,{\sf t}')$ meaning its inverse. As a final step, we can use Eqs.~(\ref{eq:master-x-cont}) and~(\ref{eq:stat-sol-kramers-moyal}) together to figure out what the conditional probability looks like
%%
\begin{align}
P_{({\sf t}+1){\sf t}'}(x\vert x') &= \frac{{\rm exp}\bigg\{ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d[x-f({\sf t}+1)]^i[K^{-1}(\theta, {\sf t}+1, {\sf t}')]^{ij}[x'-f({\sf t}')]^j\bigg\}}{(2\pi )^{\frac{d}{2}}\vert K(\theta, {\sf t}+1, {\sf t}')\vert^{\frac{1}{2}}} \label{eq:gaussian-proc-cond-p} \\
K^{ij}(\theta, {\sf t}+1, {\sf t}') &= \frac{1}{2}\sum_{k=1}^d\big[ \Sigma^{\frac{1}{2}}(\theta, {\sf t}+1)\big]^{ik}\big[ \Sigma^{\frac{1}{2}}(\theta, {\sf t}')\big]^{kj} \,,
\end{align}
%%
which is the familiar conditional probability for a kind of vector Gaussian process.\footnote{This process is very familiar in machine learning and can be a powerful tool to learn non-linear functions through deviations from the expected value.}

For Markovian phenomena, we can see that Eq.~(\ref{eq:master-x-cont}) no longer depends on timesteps older than the immediately previous one, hence the expression reduces to just
%%
\begin{align}
...
\end{align}
%%
where we note that the $P$'s in the expression above all now refer to \emph{probability mass functions}. In the case where $x$ is just a vector of binary `on' or `off' states, we can simplify this even further to 
%%
\begin{align}
P^i_{{\sf t}+1} &= \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{\sf t} \sum_{j=1}^d P^j_{{\sf t}'} P^{ij}_{({\sf t}+1){\sf t}'} \label{eq:master-x-disc-binary} \,,
\end{align}
%% 
where $P^i_{{\sf t}'}$ now represents the probability that element $x^i=1$ (is `on').
