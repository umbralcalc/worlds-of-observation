...
\end{align}
%%
So the matrix notation of Eq.~(\ref{eq:master-x-cont}) can indeed hide a very complicated calculation. Truncating just up to second order, Eq.~(\ref{eq:master-x-cont-kramers-moyal}) tells us that there can be first and second derivatives contributing to the flow of probability to each element of the row $x=X_{{\sf t}+1}$ which depend on every element of the matrix $X'$. The probability does indeed \emph{flow}, in fact. We can define a quantity known as the `probability current' $J$ to illustrate this through the continuity relation
%%
\begin{align}
P_{{\sf t}+1}(x) - \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}}P_{{\sf t}'}(x) = -\sum_{i=1}^d\frac{\partial}{\partial x^i}J^i_{{\sf t}+1}\,.
\end{align}
%%
By inspection of Eq.~(\ref{eq:master-x-cont-kramers-moyal}) we can therefore also deduce that
%%
\begin{align}
J^i_{{\sf t}+1} = \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}} \alpha^i_{({\sf t}+1){\sf t}'}(x)P_{{\sf t}'}(x) - \frac{1}{2{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}}\sum_{j=1}^d\frac{\partial}{\partial x^j}\big[ \beta^{ij}_{({\sf t}+1){\sf t}'}(x)P_{{\sf t}'}(x)\big] + \dots \,.
\end{align}
%%
...
%%
\begin{align}
\alpha^i_{({\sf t}+1){\sf t}'}(x) &= M^i_{{\sf t}+1}({\sf t}')-x^i \\
\beta^{ij}_{({\sf t}+1){\sf t}'}(x) &= K^{ij}(\theta, {\sf t}+1, {\sf t}') \,,
\end{align}
%%
where $M({\sf t}')$ is an arbitrary matrix-valued function of timestep ${\sf t}'$ and $K(\theta, {\sf t}+1, {\sf t}')$ is a covariance kernel matrix which depends on ${\sf t}+1$, ${\sf t}'$ and a set of hyperparameters $\theta$. 

\textcolor{red}{Don't need stationarity, just need to write out the terms of $$P_{{\sf t}+1}(x)-P_{{\sf t}}(x)$$ and solve the equation - should be minuses in the expression from which the conditionals can be derived!!!}


If we now also assume both stationarity of $P_{{\sf t}'}(x)=P_{{\sf t}''}(x)$ for any ${\sf t}'$ and ${\sf t}''$ such that
%%
\begin{align}
P_{{\sf t}+1}(x) = \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}} P_{{\sf t}'}(x) \,,
\end{align}
%%
and a vanishing probability current ($J^i_{{\sf t}'}=0$ for all ${\sf t}'$ and $i$), we can solve Eq.~(\ref{eq:master-x-cont-kramers-moyal}) to obtain
%%
\begin{align}
P_{{\sf t}'}(x) &= \frac{{\rm exp}\bigg\{ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d[x-M_{{\sf t}+1}({\sf t}')]^i[K^{-1}(\theta, {\sf t}+1, {\sf t}')]^{ij}[x-M_{{\sf t}+1}({\sf t}')]^j\bigg\}}{(2\pi )^{\frac{d}{2}}\vert K(\theta,{\sf t}+1, {\sf t}')\vert^{\frac{1}{2}}} \label{eq:stat-sol-kramers-moyal}\,.
\end{align}
%%

for a vector Gaussian process\footnote{This process is very familiar in machine learning and can be a powerful tool to learn non-linear functions through deviations from the expected value.} \textcolor{red}{Everything is fine up to here --- need to find a way to derive the conditional probability.}
%%
\begin{align}
P_{{\sf t}'}(x) &= {\sf GaussianProcessPDF}[x;{\sf t}+1,{\sf t}',\theta] \nonumber \\
&= \frac{{\rm exp}\bigg\{ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d[X_{{\sf t}+1}-M({\sf t}+1)]^i[K^{-1}(\theta, {\sf t}+1, {\sf t}')]^{ij}[X_{{\sf t}'}-M({\sf t}')]^j\bigg\}}{(2\pi )^{\frac{d}{2}}\vert K(\theta,{\sf t}+1, {\sf t}')\vert^{\frac{1}{2}}} \label{eq:gaussian-proc-cond-p}\,.
\end{align}
%%
Note that I write $\vert K(\theta, {\sf t}+1, {\sf t}') \vert$ meaning the determinant of the matrix and $K^{-1}(\theta, {\sf t}+1, {\sf t}')$ meaning its inverse.
