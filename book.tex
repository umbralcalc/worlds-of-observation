...
Before progressing further, it will be helpful to discuss some useful concepts that should help us analyse the system later on. The general stochastic process that we defined with Eq.~(\ref{eq:x-step-def}) also has an implicit master equation~\cite{van1992stochastic} that describes the time evolution of the probability of $X$, which can be written as
%%
\begin{align}
p_{t+1}(X) &= \int_{\Omega_t (X')}{\rm d}X' p_{t}(X') p_{t+1}(X\vert X') \label{eq:master-x-cont} \,,
\end{align}
%%
where we are assuming the state space is continuous in each dimension. If we then define the probability matrix $P$, with elements $P^i_{t}(x)$ which denote the probability of $X^i_t=x$, the latest values of this matrix are updated by each new step in time according to
%%
\begin{align}
P^i_{t+1}(x) &= \int_{\Omega_t (X')}{\rm d}X' p_{t}(X') \delta [x-F^i_{t+1}(X',t)] \label{eq:master-x-cont-update}\,,
\end{align}
%%
where we have made explicit use of Eq.~(\ref{eq:x-step-def}) and a Dirac delta distribution $\delta$.

Note that an analog of Eq.~(\ref{eq:master-x-cont}) exists for discrete state spaces where we simply replace the integral with a sum, schematically like so 
%%
\begin{align}
p_{t+1}(X) &= \sum_{\Omega_t (X')} p_t(X') p_{t+1}(X \vert X') \label{eq:master-x-disc} \,.
\end{align}
%%
