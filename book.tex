...
\caption{Graph representation of Eq.~(\ref{eq:stat-sol-kramers-moyal-cond}).}
...
\end{align}
%%
where $K(\theta , {\sf t}+1,{\sf t}')$ is some arbitrary covariance matrix that encodes how the correlation structure varies with the between compared states at two different timesteps and $K^{-1}(\theta , {\sf t}+1,{\sf t}')$ denotes taking its inverse. For a bit more visual intuition, I've illustrated what this conditional probability computation looks like in Fig~\ref{fig:stat-sol-kramers-moyal-cond}.
