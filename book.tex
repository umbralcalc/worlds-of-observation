...
& \qquad + \frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d\frac{\partial}{\partial X^i_{{\sf t}+1}}\frac{\partial}{\partial X^j_{{\sf t}+1}}[\alpha^{ij}_2 (X_{{\sf t}+1})p_{{\sf t}+1}(X_{{\sf t}+1})] + \dots \label{eq:master-x-cont-kramers-moyal} \\
...
%%
\begin{align}
\alpha^i_1(X_{{\sf t}+1}) &= \sum_{{\sf t}'=0}^{\sf t}\sum_{j=1}^d\tilde{X}^j_{{\sf t}'}M^{ij}(\theta,{\sf t}', {\sf t}+1)-X^i_{{\sf t}+1} \\
\alpha^{ij}_2(X_{{\sf t}+1}) &= \sum_{{\sf t}'=0}^{\sf t}\sum_{{\sf t}''=0}^{\sf t}\sum_{k=1}^d\sum_{\ell =1}^d(X^k_{{\sf t}'}-X^k_{{\sf t}+1})(X^\ell_{{\sf t}''}-X^\ell_{{\sf t}+1})K^{ijk\ell}(\theta, {\sf t}', {\sf t}'',{\sf t}+1) \,.
\end{align}
%%
