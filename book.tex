...
So I've proposed a computational formalism and done a bit of analysis on it to demonstrate that it can cope with a variety of different stochastic phenomena. Now I think we're ready to summarise what we want the stochadex software package to be able to do. If we begin with the obvious first set of criteria; we want to be able to freely configure the iteration function $F$ of Eq.~(\ref{eq:x-step-def}) and the timestep function $t$ of Eq.~(\ref{eq:t-steps-sum}) so that any process we want can be described. In addition, once someone has configured the stochadex in some new way, ideally we would then be able to recall this past configuration very easily in the future but with new parameters set, if desired.

The state history matrix $X$ should also be configurable in terms of its number of rows (the `state width') and its number of columns (the `state history depth'). 
