...
\end{align}
%%
where we have analogously defined $\omega_{{\sf t}}$ as the domain of $X_{{\sf t}}$ and, in this sense, $\omega_{{\sf t}}$ `exists within' $\Omega_{\sf t}$.

Given that it's only the most recent row of $X$ that gets updated in this timestep, it could also be useful to define the probability density matrix $P$ which has $x$-dependent elements $P^i_{\sf t}(x)$ --- this just means the probability density of $X^i_{\sf t}=x$. Eq.~(\ref{eq:master-x-cont}) then tells us that the latest values of this matrix are updated by a step in time according to
%%
\begin{align}
...
Technically, our definitions of $X$ and $X'$ naturally specified them to have different shapes ($X$ should have one more row than $X'$ in finite time). However, if we slightly abuse this notation for a more compact expression, it's possible to do a kind of Kramers-Moyal expansion~\cite{kramers1940brownian,moyal1949stochastic} of Eq.~(\ref{eq:master-x-cont}) to obtain
%%
\begin{align}
p_{{\sf t}+1}(X) &=  p_{{\sf t}}(X') + \sum^\infty_{n=1}\frac{(-1)^n}{n!}\frac{\partial^n}{\partial X^n}[\alpha_n (X)p_{{\sf t}+1}(X)] \label{eq:master-x-cont-kramers-moyal} \\
\alpha_n(X) &= \int_{\Omega_{\sf t}}{\rm d}X'(X'-X)^np_{\sf t}(X'\vert X,{\sf t}+1) \,,
\end{align}
%%
where $\frac{\partial^n}{\partial X^n}$ is the $n$-th order matrix derivative. Note that in order to arrive at this expression we needed to use the equivalence of $p_{{\sf t}+1}(X\vert X',{\sf t})p_{\sf t}(X') = p_{\sf t}(X'\vert X,{\sf t}+1)p_{{\sf t}+1}(X)$ by Bayes' rule. In words, $p_{\sf t}(X'\vert X,{\sf t}+1)$ is the conditional probability that $X'$ \emph{was} the previous matrix given that $X$ is the matrix in this timestep.

Let's try this expansion out on a specific example. In a vector Gaussian process, the conditional probability of the next state given the value of all of the previous states can be written as 
%%
\begin{align}
&p_{{\sf t}+1}(X_{{\sf t}+1}\vert X',{\sf t}) = \prod_{\forall X_{{\sf t}'}\in X'} (2\pi )^{-\frac{d}{2}}\vert K(\theta, X_{{\sf t}+1}, X_{{\sf t}'})\vert^{-\frac{1}{2}}{\rm exp}\Bigg[ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d\frac{f^i(X_{{\sf t}+1})f^j(X_{{\sf t}'})}{K^{ij}(\theta, X_{{\sf t}+1}, X_{{\sf t}'})}\Bigg] \,.
\end{align}
%%

In truth, the Kramers-Moyal expansion is probably most usefully employed in situations with Markovian phenomena --- otherwise, you might end up with a huge number of partial derivatives to calculate! In the Markovian case, the expansion naturally changes to
%%
\begin{align}
p_{{\sf t}+1}(X_{{\sf t}+1}) &=  p_{{\sf t}}(X_{{\sf t}}) + \sum^\infty_{n=1}\frac{(-1)^n}{n!}\sum_{i=1}^d\frac{\partial^n}{(\partial X^i_{{\sf t}+1})^n}[\alpha^i_n (X_{{\sf t}+1})p_{{\sf t}+1}(X_{{\sf t}+1})] \label{eq:markov-kramers-moyal} \\
\alpha^i_n(X_{{\sf t}+1}) &= \int_{\omega_{\sf t}}{\rm d}X_{{\sf t}+1}(X^i_{{\sf t}}-X^i_{{\sf t}+1})^np_{\sf t}(X_{{\sf t}}\vert X_{{\sf t}+1},{\sf t}+1) \,.
\end{align}
%%
