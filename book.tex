...
%%
\begin{align}
\alpha^i_1(X_{{\sf t}+1}) &= \sum_{{\sf t}'=0}^{\sf t}\sum^d_{j=1}\sum^d_{k=1}\frac{X^k_{{\sf t}+1}K^{ij}(\theta, {\sf t}', {\sf t}')}{K^{jk}(\theta, {\sf t}', {\sf t}+1)} - X^i_{{\sf t}+1} \\
\alpha^{ij}_2(X_{{\sf t}+1}) &= \sum_{{\sf t}'=0}^{\sf t}\sum^d_{k=1}\sum^d_{\ell =1}\,,
\end{align}
%%
where we have assumed that ${\sf t}'' > {\sf t}'$.

$${\rm exp}\bigg\{ -\frac{1}{2}\bigg[ X_{{\sf t}'}- \bigg(K^{-1}(\theta, {\sf t}', {\sf t}'')X_{{\sf t}''} + K^{-1}(\theta, {\sf t}', {\sf t}+1)X_{{\sf t}+1}\bigg)K(\theta, {\sf t}', {\sf t}') \bigg]K^{-1}(\theta, {\sf t}', {\sf t}')-\frac{1}{2}\frac{X^i_{{\sf t}''}X^j_{{\sf t}''}}{K^{ij}(\theta, {\sf t}'', {\sf t}'')}-\frac{1}{2}\frac{X^i_{{\sf t}'}X^j_{{\sf t}+1}}{K^{ij}(\theta, {\sf t}', {\sf t}+1)}-\frac{1}{2}\frac{X^i_{{\sf t}'}X^j_{{\sf t}''}}{K^{ij}(\theta, {\sf t}', {\sf t}'')}-\frac{1}{2}\frac{X^i_{{\sf t}''}X^j_{{\sf t}+1}}{K^{ij}(\theta, {\sf t}'', {\sf t}+1)}\bigg\}$$
