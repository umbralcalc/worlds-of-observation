...
%%
\begin{align}
P_{{\sf t}+1}(x) &= \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}}\int_{\omega_{{\sf t}'}}{\rm d}x' P_{{\sf t}'}(x') P_{{\sf t}'}(x\vert x') \label{eq:master-x-cont} \,,
\end{align}
%%
where we are assuming the state space is continuous in each dimension and $P_{{\sf t}'}(x\vert x')$ is the conditional probability that next matrix row will be $x=X_{{\sf t}+1}$ given that the row in one of the previous timesteps ${\sf t}'$ was $x'=X_{{\sf t}'}$. The factor of $1/{\sf t}$ is a normalisation factor after having applied the sum over ${\sf t}'$. Note that, if the process is defined over continuous time, we would need to replace 
%%
\begin{align}
...
The matrix notation in Eq.~(\ref{eq:master-x-cont}) can hide some staggering complexity. To try and understand this a little, we can do a kind of Kramers-Moyal expansion~\cite{kramers1940brownian,moyal1949stochastic} to approximate it like this
%%
\begin{align}
&P_{{\sf t}+1}(x) \simeq \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}}\bigg\{ P_{{\sf t}'}(x)-\sum_{i=1}^d\frac{\partial}{\partial x^i}\big[ \alpha^i_{{\sf t}'}(x)P_{{\sf t}'}(x)\big] + \frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d\frac{\partial^2}{\partial x^i\partial x^j}\big[ \beta^{ij}_{{\sf t}'}(x)P_{{\sf t}'}(x)\big]\bigg\} \label{eq:master-x-cont-kramers-moyal} \,,
\end{align}
%%
...
%%
\begin{align}
\alpha^i_{{\sf t}'}(x) &=\int_{\omega_{{\sf t}'}} {\rm d}x'(x'-x)^iP_{{\sf t}'}(x'\vert x) \\
\beta^{ij}_{{\sf t}'}(x) &= \int_{\omega_{{\sf t}'}} {\rm d}x'(x'-x)^i(x'-x)^jP_{{\sf t}'}(x'\vert x) \,.
\end{align}
%%
So the matrix notation of Eq.~(\ref{eq:master-x-cont}) can indeed hide a very complicated calculation. Eq.~(\ref{eq:master-x-cont-kramers-moyal}) tells us that there can be first and second derivatives contributing to the flow of probability to each element of the row $x=X_{{\sf t}+1}$ which depend on every element of the matrix $X'$. \textcolor{red}{Introduce the probability current here to help the description.}

For Markovian phenomena, note that Eq.~(\ref{eq:master-x-cont}) simplifies to
%%
\begin{align}
P_{{\sf t}+1}(x) &= \int_{\omega_{\sf t}}{\rm d}x' P_{\sf t}(x') P_{{\sf t}}(x\vert x') \label{eq:master-x-cont-markov} \,.
\end{align}
%%
...
What would happen if we tried to approximate $\alpha$ and $\beta$ with simpler, time-dependent functions? For example, let's make the following assumptions
%%
\begin{align}
\alpha^i_{{\sf t}'}(x) &= M^i({\sf t}')-x^i \\
\beta^{ij}_{{\sf t}'}(x) &= K^{ij}(\theta, {\sf t}+1, {\sf t}') \,,
\end{align}
%%
where $M({\sf t}')$ is an arbitrary vector-valued function of timestep ${\sf t}'$ and $K(\theta, {\sf t}+1, {\sf t}')$ is a covariance kernel matrix which depends on ${\sf t}'$, the next timestep ${\sf t}+1$ and also on a set of hyperparameters $\theta$. If we also assume stationarity of $P_{{\sf t}'}(x)=P_{{\sf t}''}(x)$ for any ${\sf t}'$ and ${\sf t}''$ such that
%%
\begin{align}
P_{{\sf t}+1}(x) = \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}} P_{{\sf t}'}(x) \,,
\end{align}
%%
and plug both this and the time-dependent functions back into Eq.~(\ref{eq:master-x-cont-kramers-moyal}), we can solve the equation to obtain the probability density for a vector Gaussian process\footnote{This process is very familiar in machine learning and can be a powerful tool to learn non-linear functions through deviations from the expected value.}
%%
\begin{align}
P_{{\sf t}'}(x) &= {\sf GaussianProcessPDF}[x;{\sf t}+1,{\sf t}',\theta] \\
&= (2\pi )^{-\frac{d}{2}}\vert K(\theta,{\sf t}+1, {\sf t}')\vert^{-\frac{1}{2}}{\rm exp}\Bigg[ -\frac{1}{2}\sum_{i=1}^d\sum_{j=1}^d\frac{X^i_{{\sf t}+1}X^j_{{\sf t}'}}{K^{ij}(\theta, {\sf t}+1, {\sf t}')}\Bigg] \label{eq:gaussian-proc-cond-p}\,.
\end{align}
%%
Note that I write $\vert K(\theta, {\sf t}+1, {\sf t}') \vert$ meaning the determinant of the matrix. \textcolor{red}{Point out that the probability current is zero.}

This example was just for illustration rather than for thinking about actually using the Gaussian process.

Note that an analog of Eq.~(\ref{eq:master-x-cont}) exists for discrete state spaces as well. In the case of Eq.~(\ref{eq:master-x-cont}), for instance, we would simply replace the integral with a sum and the schematic would look something like this
%%
\begin{align}
