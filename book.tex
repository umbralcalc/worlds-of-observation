...
\end{align}
%%
where $M({\sf t}')$ is an arbitrary matrix-valued function of timestep ${\sf t}'$ and $K(\theta, {\sf t}+1, {\sf t}')$ is a covariance kernel matrix which depends on ${\sf t}+1$, ${\sf t}'$ and a set of hyperparameters $\theta$. If we also assume stationarity of $P_{{\sf t}'}(x)=P_{{\sf t}''}(x)$ for any ${\sf t}'$ and ${\sf t}''$ such that
%%
\begin{align}
P_{{\sf t}+1}(x) = \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}} P_{{\sf t}'}(x) \,,
\end{align}
%%
and plug both this and the time-dependent functions back into Eq.~(\ref{eq:master-x-cont-kramers-moyal}), we can solve the equation to obtain the probability density for a vector Gaussian process\footnote{This process is very familiar in machine learning and can be a powerful tool to learn non-linear functions through deviations from the expected value.} \textcolor{red}{Everything is fine up to here --- need to find a way to derive the conditional probability.}
%%
\begin{align}
