...
\emph{Diffusing ideas} has been written to explore a collection of interrelated ideas around the statistical inference and control of an extremely general class of stochastic phenomena. In order to manifest these ideas into reality, this project has also led to designing and building some new, open-source, scientific software tools that form a foundation from which one can build a variety of new applications. Testing and experimentation has also given us the opportunity to develop and play with a range of fun and interesting mathematical toy models which are motivated by the real world --- the potential examples are vast, and we encourage the reader to think of more!

Any software that was developed in this book will always have been shared under a \href{https://opensource.org/licenses/MIT}{MIT license} in the following public git repository for anyone to use: \href{https://github.com/umbralcalc}{https://github.com/umbralcalc}. All of this sofware should also still be considered as very much `in development' and so it is advised that it is not deployed in production without careful scrutiny at the user's own risk.

No quest would be complete without a map, so in this introductory section we will now outline some of the key milestones of the project and their motivations within the context of the overall research ideas. 

\begin{enumerate} 
\item{How do we generate a very general set of stochastic phenomena?}
\item{How do we learn/identify these phenomena from real-world data?}
\item{How do we generate a very general set of control policies to interact with these phenomena?}
\item{How do we then learn the optimal policies to achieve a specified control objective?} 
\end{enumerate}
