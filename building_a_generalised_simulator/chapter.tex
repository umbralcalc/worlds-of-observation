...
It's worth noting that while Fig.~\ref{fig:loop-design} illustrates only a single process; it's obviously true that we may run many of these whole diagrams at once to parallelise generating independent realisations of the simulation, if necessary.

\textcolor{red}{
\begin{itemize}
\item{talk about the frontend here and its uses}
\item{introduce and describe the schematic diagram for the main executable}
\end{itemize}}
