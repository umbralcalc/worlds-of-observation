...
// returns the state vector from the S(X',z,t) function we defined 
...
// returns the time-inhomogeneous Poisson process S(X',z,t) term 
func SFunctionInhomogeneousPoissonProcess(
    stateHistory StateHistory,
    otherParams OtherParams,
    timeStepNumber int,
) StateVector {
    // get this from otherParams
...
// returns the Hawkes process S(X',z,t) term
...
S^{1}_{{\sf t}+1}(X',z,{\sf t}) &= \frac{1}{T}\sum^{{\sf t}}_{{\sf t}'=0}e^{-\frac{t({\sf t})-t({\sf t}')}{T}} S^{0}_{{\sf t}'}\,,
\end{align}
%%
