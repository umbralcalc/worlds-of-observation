...
The empirical emulator algorithms that we're going to describe in this section will depend on the stationarity of $P_{{\sf t}+1}(x)=P_{{\sf t}'}(x)$ such that, e.g., Eq.~(\ref{eq:stationary-mean-estimator}) is applicable. But before we think about the various kinds of conditional probability we could use, we need to connect values of $x$ in our emulator to the data from which it will be learning. 

\textcolor{red}{Need to go through the basic types of data distribution here... Exponential, Poisson, Binomial, Gaussian, Gamma, Negative Binomial, ...}

One clear, and generally applicable, option for the conditional probability is that of a Gaussian process 
%%
\begin{align}
P_{({\sf t}+1){\sf t}'}(x\vert x') &= \frac{{\rm exp}\left\{ -\frac{1}{2}\sum^d_{i=1}\sum^d_{j=1}(x-f)^i \big[ K^{-1}(\theta , {\sf t}+1,{\sf t}')\big]^{ij}(x'-f')^j  \right\}}{\vert 2\pi K(\theta , {\sf t}+1,{\sf t}') \vert^{\frac{1}{2}}} \label{eq:gaussian-process-cond-prob}\,,
\end{align}
%%
where $\vert A\vert$ denotes computing the determinant of matrix $A$.
