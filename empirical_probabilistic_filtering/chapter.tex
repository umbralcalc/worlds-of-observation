...
// returns the current mean given a conditional probability weighting over 
// past data vectors in the input history
func ComputeFilteredMean(dataHistory DataHistory) MeanVector {
    cumulativeWeightsSum := 0.0
    meanVector := make([]float64, len(dataVector[0]))
    for i, dataVector := range dataHistory {
        // ignore first (most recent) value in the history as this is the one 
        // we want to compare to in the log-likelihood
        if i == 0 {
            continue
        }
        weight := ConditionalProbability(dataHistory[0], dataVector)
...
// compute the overall log-likelihood for the most recent point found in the 
// input the data history
func LogLikelihood(
    ...
    mean := ComputeFilteredMean(dataHistory)
    logLikelihood := DataLinkingLogProbability(dataVector, mean)
