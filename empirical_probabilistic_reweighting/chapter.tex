...
%%
\begin{align}
M_{{\sf t}+1}(z) = \int_{\omega_{{\sf t}+1}}{\rm d}x \,x\,P_{{\sf t}+1}(x\vert z) &= \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}}\int_{\omega_{{\sf t}'}}{\rm d}x' \, x' \, P_{{\sf t}'}(x'\vert z) \int_{\omega_{{\sf t}+1}}{\rm d}x \, P_{({\sf t}+1){\sf t}'}(x\vert x',z) \label{eq:stationary-mean-estimator}\,.
\end{align}
%%
...
&= \frac{1}{{\sf t}}\sum_{{\sf t}'=0}^{{\sf t}}\int_{\omega_{{\sf t}'}}{\rm d}x' \, [x'-M_{{\sf t}+1}(z)]^i[x'-M_{{\sf t}+1}(z)]^j P_{{\sf t}'}(x'\vert z) \int_{\omega_{{\sf t}+1}}{\rm d}x \, P_{({\sf t}+1){\sf t}'}(x\vert x',z) \label{eq:stationary-covariance-estimator}\,.
\end{align}
%%
...
%%
\begin{align}
{\sf weight}(x',z) \propto \int_{\omega_{{\sf t}+1}} {\rm d}x\, P_{({\sf t}+1){\sf t}'}(x\vert x',z) \,,  
\end{align}
%%
we can apply this as a \emph{reweighting} of the historical time series samples to estimate any statistics of interest. Taking Eq.~(\ref{eq:stationary-mean-estimator}) as the example; we are essentially approximating this integral through a weighted sample estimation like this
%%
\begin{align}
M_{{\sf t}+1}(z) \simeq \frac{\sum_{\forall x' \in {\sf data}}x' \, {\sf weight}(x',z)}{\sum_{\forall x' \in {\sf data}} {\sf weight}(x',z)} \,.
\end{align}
%%
Our goal in this section will be to learn the optimal reweighting ${\sf weight}(x',z)$ with respect to $z$, i.e., the one which most accurately represents a provided dataset. But before we think about the various kinds of conditional probability we could use, we need to think about how to connect the estimated statistics from the reweighting to the data by defining an objective function. 
