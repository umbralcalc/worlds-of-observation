...
\item{the online algorithm is specifically: 1. sample from the $z$ distribution using the current variance estimate, keeping only the current MAP sample and replacing all of the other samples 2. run the iterations again from the window size back in time ${\sf t}'={\sf s}$ all the way up to ${\sf t}'={\sf t}+1$ 3. determine the new MAP sample through Eq.~(\ref{eq:windowed-z-update}) and Eq.~(\ref{eq:likelihhood-free-posterior}), then recompute the estimated variance of the other samples around it using the importance weightings 4. iterate time forward, stream a new datapoint in, recompute the likelihood statistics to go into Eq.~(\ref{eq:likelihhood-free-posterior}) and go to 1. }
\end{itemize}}
