...
\textcolor{red}{Note that the reweighting algorithm can be used for not just $P_{{\sf t}+1}[\,{\sf y};M_{{\sf t}+1}(Y),C_{{\sf t}+1}(Y), \dots ]$ but also $P_{{\sf t}+1}(x,z\vert {\sf Y}_{{\sf t}+1}, {\sf Y}_{{\sf t}}, \dots )$. If these statistics are estimated by a neural network then it can be quickly retrained to generate new samples. Also talk about extensions which use neural networks not just on the compression (or `encode') step but also use them as a layer between the sample from the data distribution model and the output (the `decode' step) --- this is also known as a Variational Autoencoder (VAE)~\cite{pinheiro2021variational}.}
