...
%%
\begin{align}
P_{{\sf t}+1}({\cal Y}_{{\sf t}+1} \vert z) = \int_{\omega_{{\sf t}+1}}{\rm d}x\, P_{{\sf t}+1}({\cal Y}_{{\sf t}+1} \vert x)P_{{\sf t}+1}(x\vert z) \,,
\end{align}
%%
where, in practical terms, the measurement $P_{{\sf t}+1}({\cal Y}_{{\sf t}+1} \vert x)$ can be represented by sampling from another stochastic process which takes the state of the stochadex simulation as input. Given that we have this capability to compare like-for-like between the data and the simulation; the first problem is to figure out how this comparison between two sequences of vectors can be done in a way which ensures the the statistics of the posterior are ultimately respected. 
...
%%
\begin{align}
{\cal P}_{{\sf t}+1}(z\vert Y) = {\cal P}_{{\sf t}+1}(z \vert {\cal Y}_{{\sf t}+1}, Y_{{\sf t}+1}){\cal P}_{{\sf t}}(z\vert  {\cal Y}_{{\sf t}}, Y_{{\sf t}}) \dots \,.
\end{align}
%%
...
{\cal P}^{n+1}_{{\sf t}+1}({\cal Y}_{{\sf t}+1} \vert z, Y_{{\sf t}}, Y_{{\sf t}-1}, \dots) &= \int_{\omega_{{\sf t}+1}}{\rm d}x\, P_{{\sf t}+1}({\cal Y}_{{\sf t}+1} \vert x){\cal P}^{n+1}_{{\sf t}+1}(x\vert z, Y_{{\sf t}}, Y_{{\sf t}-1}, \dots) \\
{\cal P}^{n+1}_{{\sf t}+1}(z \vert {\cal Y}_{{\sf t}+1}, Y_{{\sf t}+1}) &= \frac{1}{n}\sum_{n'=0}^n\int_{\upsilon_{n'}} \,{\rm d}z' \, P^{(n+1)n'}(z\vert z')\, {\cal P}^{n'}_{{\sf t}+1}(z'\vert {\cal Y}_{{\sf t}+1}, Y_{{\sf t}+1}) \label{eq:posterior-process} \,,
\end{align}
%%
