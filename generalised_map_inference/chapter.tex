...
\Rightarrow P_{{\sf t}+1}(z\vert {\sf Y}) &\propto P_{0}(z) \prod_{{\sf t}'=0}^{{\sf t}}\bigg[ \int_{\omega_{{\sf t}'+1}} {\rm d}x \, P_{{\sf t}'+1}({\sf y}\vert x) P_{{\sf t}'+1}(x\vert z) \bigg] \,,
\end{align}
%%
shows how the $z$ updates can occur in an iterative fashion. The reader may also recognize the factor above in brackets as Eq.~(\ref{eq:simulation-measurement}).
