...
%%
\begin{align}
P_{{\sf t}+1}({\sf Y} \vert z) = \frac{1}{{\sf t}+1}\sum_{{\sf t}'=0}^{{\sf t}+1}\int_{\omega_{{\sf t}'}}{\rm d}^nx\, P_{{\sf t}'}({\sf y} \vert x)P_{{\sf t}'}(x \vert z) \,, \label{eq:simulation-measurement}
\end{align}
%%
where, in practical terms, the measurement probability $P_{{\sf t}'}({\sf y} \vert x)$ of ${\sf Y}_{{\sf t}'}={\sf y}$ given $X_{{\sf t}'}=x$ can be represented by sampling from another stochastic process which takes the state of the stochadex simulation as input. Given that we have this capability to compare like-for-like between the data and the simulation; the next problem is to figure out how this comparison between two sequences of vectors can be done in a way which ensures the the statistics of the posterior are ultimately respected. 
...
achieved by running lots of simulations and solving the inverse problem with the y outputs
