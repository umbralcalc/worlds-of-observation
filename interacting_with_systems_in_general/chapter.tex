...
\end{align}
%%
where we recall that $P_{({\sf t}+1){\sf t}}(X\vert X',z,A)$ is the conditional probability of $X=X_{0:{\sf t}+1}$ given $X'=X_{0:{\sf t}}$ and $z$ that we have encountered before, but it now requires $A=A_{0:{\sf t}+1}$ as another given input. We have illustrated Eq.~(\ref{eq:generalised-state-actions}) and how it relates to the policy distribution of Eq.~(\ref{eq:joint-prob-x-and-a}) with a new graph representation in Fig.~\ref{fig:fundamental-loop-with-actions}.
