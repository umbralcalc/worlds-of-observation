...
\begin{figure}[h]
\centering
\includegraphics[width=10cm]{images/chapter-9-fundamental-loop-with-actions.drawio.png}
...
Let us now define a fully general Non-Markovian Decision Process (NMDP) as a probabilistic model which draws an actions matrix $A$ from the following `policy' distribution
%%
\begin{align}
P_{{\sf t}+1}(A\vert z,\theta ) &= \int_{\Omega_{{\sf t}}} {\rm d}X P_{{\sf t}}(X\vert z,A')\, \Pi_{({\sf t}+1){\sf t}}(A\vert X,\theta) \,.
\end{align}
%%
...
%%
\begin{align}
P_{{\sf t}+1}(X\vert z,\theta ) &= \int_{\Omega_{{\sf t}}}{\rm d}X' P_{{\sf t}}(X'\vert z, A') \,\Pi_{({\sf t}+1){\sf t}}(A\vert X',\theta) P_{({\sf t}+1){\sf t}}(X\vert X',z,A) \,,
\end{align}
%%
