...
To model case 1. above, the simplest option would be to associate the attempt at goal with a goal success probability for the kicker $p_{\rm goal}(X_{\sf t})$ which also depends on the location on the pitch from which the kick is taken. 

Lastly, to model case 3. above, the event has determined that the ball will leave the field of play and so the remaining unknowns that need to be determined are: which side of the field this occurred (we'll just give this a binomial probability of 0.5 for either side), the longitudinal distance of the kick and whether or not the ball bounced before leaving the field. For simplicity, let's determine the last of these through another kind of kick accuracy probability $p_{\rm kick}$ associated to the kicker. This just leaves the longitudinal distance that the kick achieved along the touchline; in this case we'll just assign the probability density $P_{{\sf t}+1}(\ell )$ of $L^{\rm lon}_{{\sf t}+1}-L^{\rm lon}_{{\sf t}}=\ell$ to
%%
\begin{align}
P_{{\sf t}+1}(\ell ) = {\sf NormalPDF}[\ell; \mu_{\rm kick}(X_{\sf t})-L^{\rm lon}_{{\sf t}}, \sigma_{\rm kick}^2(X_{\sf t})] \label{eq:longitudinal-kick-rugby} \,,
\end{align}
%%
where $\mu_{\rm kick}(X_{\sf t})$ and $\sigma_{\rm kick}(X_{\sf t})$ are parameters which can vary with location of the ball on the pitch and can be determined by general trends in the tactical patterns of a rugby match.
