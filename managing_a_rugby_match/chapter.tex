...
In the case of a {\sf Run Phase}, we will choose the longitudinal component (lengthwise along the pitch) of the ball location $L^{\rm lon}_{{\sf t}}$ to be updated by the difference between samples drawn from two gamma distributions (one associated to each team). Hence, the probability density $P_{{\sf t}}(\ell )$ of $L^{\rm lon}_{{\sf t}}=\ell$ at timestep ${\sf t}$ evolves according to
%%
\begin{align}
P_{{\sf t}+1}(\ell ) = \int^\infty_0 {\rm d}\ell'\, {\sf GammaPDF}(\ell + \ell'; k_{\rm att}, \theta_{\rm att}){\sf GammaPDF}(\ell'; k_{\rm def}, \theta_{\rm def}) \,,
\end{align}
%%
where $(k_{\rm att}, \theta_{\rm att})$ are the gamma distribution parameters for the attacking team; $(k_{\rm def}, \theta_{\rm def})$ are the gamma distribution parameters for the defending team; and we have chosen positive values of $\ell$ to be aligned with the forward direction for the attacking team. We shall elaborate on how these gamma distribution parameters are calculated when we discuss associating events for player abilities in due course.

\textcolor{red}{Need to discuss: lateral motion for run phases and both longitudinal and lateral motion for kick phases.}

Note that because the ball location is a part of the overall game state, it will be included as information contained within some elements of $X_{{\sf t}}$ as well. To make this explicit, we can simply set $X^2_{{\sf t}}=L^{\rm lon}_{{\sf t}}$ and $X^3_{{\sf t}}=L^{\rm lat}_{{\sf t}}$.
