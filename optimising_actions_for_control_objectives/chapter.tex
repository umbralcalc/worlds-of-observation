...
&= \sum_{{\sf t}'={\sf t}+1}^{\infty} \int_{\omega_{{\sf t}'}}{\rm d}^nx'\int_{\rho_{{\sf t}'}} {\rm d}r \,r\, \prod_{{\sf t}''={\sf t}}^{{\sf t}'} \gamma [\delta t({\sf t}''+1)]P_{({\sf t}''+1){\sf t}''}(r,x'\vert X, z,\theta) \,.
\end{align}
%%

\textcolor{red}{Figure out how to get the gamma factor to be 1 in front of the first term in the product expression above...}
...
%%
\begin{align}
V_{{\sf t}}(X,z,\theta) &= \int_{\omega_{{\sf t}+1}}{\rm d}^nx\int_{\rho_{{\sf t}+1}} {\rm d}r \, P_{({\sf t}+1){\sf t}}(r,x'\vert X, z,\theta)\bigg\{ r+\gamma [\delta t({\sf t}+1)]V_{{\sf t}+1}(X',z,\theta)\bigg\} \,,
\end{align}
%%
