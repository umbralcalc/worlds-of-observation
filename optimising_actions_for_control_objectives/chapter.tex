...
%%
\begin{align}
V_{{\sf t}}(X,z,\theta) &= \frac{1}{{\cal N}_{{\sf t}}}\sum_{{\sf t}'={\sf t}}^{\infty} \int_{\Omega_{{\sf t}'}}{\rm d}X'\int_{\rho_{{\sf t}'}} {\rm d}r \,r\, \prod_{{\sf t}''={\sf t}}^{{\sf t}'} \gamma [\delta t({\sf t}''+1)]P_{({\sf t}''+1){\sf t}''}(r,X'\vert X, z,\theta) \,,
\end{align}
%%
where ${\cal N}_{{\sf t}}$ is a normalisation factor that can be computed from
%%
\begin{align}
{\cal N}_{{\sf t}} &= \sum_{{\sf t}'={\sf t}}^{\infty}\left\{ \prod_{{\sf t}''={\sf t}}^{{\sf t}'} \gamma [\delta t({\sf t}''+1)] \right\} \,.
\end{align}
%%
The idea behind this discount factor $\gamma$ is to decrease the contribution of rewards to the optimisation objective (often called the `expected return' in RL) more and more as the prediction increases into the future. Note also that $V$ is inherently recursively defined, such that
%%
\begin{align}
...
\end{align}
%%
Using $\theta^*_{{\sf t}}(X,z)$ the optimal state value function and policy distribution can also be defined
%%
\begin{align}
