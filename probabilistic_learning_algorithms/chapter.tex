...
Another probabilistic learning technique which we are now ready to discuss is what we shall call `empirical probabilistic reweighting'. While we think that it's worth going into some mathematical detail to give a better sense of where this technique comes from; we want to emphasise that the methodology we discuss here is not new to the technical literature at all. You could argue this method draws on influences from Empirical Dynamical Modeling (EDM)~\cite{sugihara1990nonlinear}, some classic nonparametric local regression techniques --- such as LOWESS/Savitzky-Golay filtering~\cite{savitzky1964smoothing} --- and also Gaussian processes~\cite{murphy2012machine} to some extent as well. 

There is another similar expression for the mean that we can derive under certain conditions which will be valuable to us when conceptualising the empirical reweighting algorithm. If the probability distribution over each row of the state history matrix is \emph{stationary} --- meaning that $P_{{\sf t}+1}(x\vert z)=P_{{\sf t}'}(x\vert z)$ --- it's possible to go one step further than Eq.~(\ref{eq:mean-field-master}) and assert that
%%
\begin{align}
